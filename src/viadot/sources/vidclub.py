...
from ..utils import handle_api_response, add_viadot_metadata_columns
...
class VidclubCredentials(BaseModel):
    ...
    Description:
        Credentials.
    Keys:
       token: str  # The token required by the API
    """
    token: str
...
class Vidclub(Source):
    ...
    def total_load(
        ...
        """
        Loops get_response and iterates by date ranges defined in intervals. Stores outputs as DataFrames in a list.
        Results in a concatenation of the dataframes when the duplicates that may appear when querying are dropped.
        ...
        if len(starts) > 0 and len(ends) > 0:
            for start, end in zip(starts, ends):
                ...
                print(f"Ingesting data for dates [{start}]-[{end}]...")
        ...
        if df.empty:
            ...
            print(f"{Fore.RED}ERROR:{Style.RESET_ALL} No data for this date range")
    ...
    def to_df(self,
        ...
        region: Literal["bg", "hu", "hr", "pl", "ro", "si", "all"] = '',
        from_date:  str = "2022-03-22",
        to_date: str = '',
        items_per_page: int = 100,
        if_empty: str = "fail"
        ...
        ValidationError: Creates an exception depending on context due to issues in imputed data..
        ...
        if df.empty:
            raise ValueError("No data collected from the data source!")
